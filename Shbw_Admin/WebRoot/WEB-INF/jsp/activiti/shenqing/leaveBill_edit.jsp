...
<script type="text/javascript">
	...
	function save(lcjd){
		...
		$.ajax({
			type: "POST",
			url: '<%=basePath%>leaveBill/editKpls.do',
	...
	function searchDd(){
		...
		var ddid = $("#dingDanIds").val();
		$("#showOrhidden").hide();
		$("#cplb").val("0");
		var diagTt = new top.Dialog();
		diagTt.Drag = true;
		diagTt.Title = "订单信息添加";
		diagTt.URL = '<%=basePath%>leaveBill/searchDd.do?ddid='+ddid+'&fplx='+fplx+'&xfid='+xfid;
		diagTt.Width = 1100;
		diagTt.Height = 600;
		diagTt.CancelEvent = function() { // 关闭事件
...
<body>
	<form action="leaveBill/save.do" name="userForm" id="userForm" method="post">
		...
		<input type="hidden" name="ttxxid" id="ttxxid" value="${ttxxid}"/>
		...
		<table style="margin:auto;">
			...
			<select name="gfmc" id="gfmc" data-live-search="true" data-live-search-style="begins"  title="请选择发票抬头">
				<c:if test="${fptt!=null}"><option value="${fptt.id }">${fptt.gfmc }</option></c:if>
			</select>
			<span class="portmsg">*</span>
			...
			<tr>
				<td><label id="cplbtips">产品类别：</label></td>
					<td><select id="cplbTemp" name="cplbTemp" title="--请选择产品类型--">
			...
			<tr id="showOrhidden">
				...
				<td><input type="text" name="zhbh" value="${pdKpls.zhbh }" id="zhbh" maxlength="128" placeholder="账户编码" title="账户编码"/></td>
			</tr>
			<tr>
				...
				<td><input type="text" name="ykpje" value="${pdKpls.ykpje }" id="ykpje" <c:if test="${0==pdKpls.cplb}">readonly="readonly"</c:if> onblur="checkYkpje()" maxlength="20" placeholder="开票金额" title="开票金额"/><span class="portmsg">*</span></td>
			</tr>
			<tr>
				<td><label>收 件  人：</label></td>
				<td><input type="text" name="gflxr" maxlength="32" id="gflxr" value="${pdKpls.gflxr }" placeholder="收 件  人" title="收 件  人"/><span class="portmsg">*</span></td>
				<td><label>收件人电话：</label></td>
				<td><input type="text" name="gflxrdh" maxlength="32" id="gflxrdh" value="${pdKpls.gflxrdh }" placeholder="收件人电话" title="收件人电话"/><span class="portmsg">*</span></td>
			</tr>
			<tr>
